#!/usr/bin/env python3
"""
Compute the angle in parameter space for three checkpoints: endpoint A, checkpoint C, endpoint B.

This script was adapted to automatically handle CurveNet "curve-format" checkpoints
(which store per-bend parameters like 'net.conv1.weight_0', 'net.conv1.weight_1', ...).
If a checkpoint appears to be curve-format the script will extract a single bend
(control point) and convert it to a plain base-model-style state dict before
vectorizing parameters.

Quick description
- Treats model parameters as vectors in a high-dimensional space by concatenating matching tensors from checkpoints.
- Computes the angle at C between A and B using vectors (A - C) and (B - C):
    angle = arccos( ((A-C)·(B-C)) / (||A-C|| * ||B-C||) )

What the script outputs
- number of concatenated parameters
- ||A - C||, ||B - C||, dot((A-C),(B-C))
- cosine similarity, angle in radians and degrees
- optional per-key contributions and top-K list

New/important behavior
- If any checkpoint looks like a CurveNet checkpoint (contains keys under 'net.' and per-bend suffixes like _0),
  it will be converted automatically to base-key format by extracting the specified bend index (default: middle).
- Use --curve-bend-index to select which bend index to export from curve checkpoints.
- By default only floating tensors are concatenated (parameters and floating buffers). Integer buffers such as
  'num_batches_tracked' are ignored unless you use --include-buffers.

Usage examples
1) Basic:
   python param_angle.py -a /path/A.pt -c /path/C.pt -b /path/B.pt

2) If C is stored as a CurveNet checkpoint (3 bends) and you want to use the middle bend:
   python param_angle.py -a A.pt -c C_curve.pt -b B.pt --curve-bend-index 1

3) Show counts, top 20 contributing keys and save summary CSV:
   python param_angle.py -a A.pt -c C.pt -b B.pt --show_counts --topk 20 --save_csv angle_summary.csv

4) Save JSON and per-key contributions:
   python param_angle.py -a A.pt -c C.pt -b B.pt --save_json summary.json --save_contribs contribs.csv

Notes
- Checkpoints saved by train.py typically contain a 'model_state' key; this script handles that automatically.
- If a curve checkpoint uses a different naming convention you can still inspect keys (print with the helper earlier)
  and then run this script with an appropriate --curve-bend-index.
"""
